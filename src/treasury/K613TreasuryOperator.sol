...
/// @dev Why a weekly swap must not go through the Safe: `minK613Out` is computed against the price
...
contract K613TreasuryOperator is AccessControl {
    ...
    // Monad mainnet, see docs/OPERATIONS_SOP.md D.1/D.3
    Treasury public constant TREASURY = Treasury(0x3377BAB9A510A586627D2f9013e132d269Eb9871);
    ...
    function minOutFloor(uint256 amountIn) public view returns (uint256) {
        ...
        uint256 atFeedPrice = (amountIn * 1e20) / uint256(answer);
        return (atFeedPrice * (10_000 - maxSlippageBps)) / 10_000;
    }
    ...
    function setPriceGuard(address priceFeed_, uint256 maxSlippageBps_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if (maxSlippageBps_ > 1_000) revert MinOutTooLow(maxSlippageBps_, 1_000);
        priceFeed = AggregatorInterface(priceFeed_);
        maxSlippageBps = maxSlippageBps_;
        emit PriceGuardUpdated(priceFeed_, maxSlippageBps_);
    }
