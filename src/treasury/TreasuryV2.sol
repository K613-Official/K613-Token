...
import {StakingV2} from "../staking/StakingV2.sol";

/// @title Treasury V2
/// @notice Manages K613 token flows: stakes K613 to get xK613 for rewards, executes buybacks. Rewards are distributed in xK613.
/// @dev Identical to V1 except it points at `StakingV2` instead of `Staking`. A new Treasury is
///      required for the Staking migration because V1's `staking` reference is `immutable` — there
///      is no setter, by design, so it can never be repointed after deploy. See `StakingV2`'s
///      NatSpec for why the migration exists and the required role sequencing (both Staking
///      versions need `MINTER_ROLE` on xK613 in parallel until V1's exit queue fully drains).
///
///      `stakeForExternalIncentives` no longer needs a `systemStaker` registration on Staking —
///      V2 has no such concept. Treasury just holds an ordinary xK613 balance and, if it ever
///      needed to unwind it, would exit through the same queue as anyone else.
contract TreasuryV2 is AccessControl, Pausable, ReentrancyGuard {
    ...
    /// @notice Staking contract this Treasury converts K613 through. Deliberately mutable.
    /// @dev V1 had this immutable, which is why a staking migration forced a Treasury redeploy —
    ///      and a Treasury redeploy in turn forces a new `PullRewardsTransferStrategy` (its
    ///      `REWARDS_VAULT` is immutable) and a re-`configureAssets` across every incentivised
    ///      market. One frozen address cascaded into reconfiguring the entire lending incentive
    ///      layer. A setter here stops that cascade: the Treasury address, and therefore the
    ///      rewards vault every strategy points at, survives future staking swaps untouched.
    ...
    /// @notice Emitted when the Treasury is repointed at a new Staking contract.
    event StakingUpdated(address indexed previousStaking, address indexed newStaking);

    /// @notice Emitted when K613 is pulled in, staked, and forwarded to RewardsDistributor as xK613.
    /// @dev Distinct from `StakedForExternalIncentives`: both stake K613, but this one hands the
    ///      resulting xK613 to the distributor's reward stream, while that one leaves it on this
    ///      balance for the pull strategy. An indexer that cannot tell them apart will misattribute
    ///      the reward stream.
    /// @param from Account the K613 was pulled from.
    /// @param amount K613 staked and forwarded as xK613.
    ...
    /// @notice Deploys the Treasury with K613, xK613, StakingV2, and RewardsDistributor.
    ...
    /// @dev    The resulting xK613 sits on Treasury's balance and is later pulled by the strategy via
    ///         `transferFrom` (see `approveXk613PullRewards`). This contrasts with `depositRewards`, which
    ///         forwards xK613 to `RewardsDistributor` and adds to its reward stream.
    ...
    /// @notice Repoints this Treasury at a new Staking contract. Only DEFAULT_ADMIN_ROLE.
    /// @dev Safe to call at any point: every K613 approval this contract grants Staking is set and
    ///      zeroed inside the same call (`stakeForExternalIncentives`, `depositRewards`), so no
    ///      standing allowance is left behind for the outgoing address.
    /// @dev What this does NOT migrate is the xK613 already on this balance — it was minted by the
    ///      previous Staking and remains claimable only against whatever backs it. Repoint first,
    ///      then settle that stock deliberately; the two are separate decisions.
    /// @param staking_ New Staking contract. Must be non-zero.
    function setStaking(address staking_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (staking_ == address(0)) revert ZeroAddress();
        emit StakingUpdated(address(staking), staking_);
        staking = StakingV2(staking_);
    }
