...
/// @notice Mechanical port of Treasury.t.sol to TreasuryV2 — the two contracts differ only in the
///         type of their `staking` reference (see TreasuryV2's NatSpec for why a redeploy was
///         unavoidable: V1's is `immutable`). Every assertion is unchanged, which is itself the
///         point: the migration must not alter any Treasury behavior.
contract TreasuryV2Test is Test {
    ...
    function testStakeForExternalIncentives_ZeroAmountReverts() public {
        vm.expectRevert(TreasuryV2.ZeroAmount.selector);
        treasury.stakeForExternalIncentives(0);
    }

    /// @notice testStakeForExternalIncentives_StakesAndKeepsXk613OnTreasury: TreasuryV2 K613 -> xK613 1:1, xK613 stays on TreasuryV2.
    function testStakeForExternalIncentives_StakesAndKeepsXk613OnTreasury() public {
        // setUp seeded TreasuryV2 with 1000 K613.
        ...
        assertEq(xk613.balanceOf(address(treasury)), xk613Before + amount, "xK613 lands on TreasuryV2 (NOT RD)");
        assertEq(staking.totalBacking(), backingBefore + amount, "StakingV2.totalBacking grows by amount");
    }
    ...
    /// @notice testStakeForExternalIncentives_PausedReverts: TreasuryV2 paused -> stakeForExternalIncentives reverts.
    ...
    function testStakeForExternalIncentives_EmitsEvent() public {
        ...
        emit TreasuryV2.StakedForExternalIncentives(100 * ONE);
        treasury.stakeForExternalIncentives(100 * ONE);
    }

    /// @notice testStakeForExternalIncentives_PullStrategyFlow: end-to-end — after stake + approve, a "strategy" address can transferFrom xK613 from TreasuryV2.
    ...
    function testStakeForExternalIncentives_PullStrategyFlow() public {
        ...
        assertEq(xk613.balanceOf(address(treasury)), stakeAmount - claimAmount, "TreasuryV2 debited");
    }
    ...
    function testApproveXk613PullRewards_ZeroSpenderReverts() public {
        vm.expectRevert(TreasuryV2.ZeroAddress.selector);
    ...
    function testApproveXk613PullRewards_SuccessAndRevoke() public {
        ...
        emit TreasuryV2.Xk613PullAllowanceSet(spender, type(uint256).max);
    ...
    function testSetRouterWhitelist_SuccessAndBuybackV3() public {
        ...
        emit TreasuryV2.RouterWhitelistUpdated(address(router), true);
        treasury.setRouterWhitelist(address(router), true);
        ...
        assertFalse(treasury.routerWhitelist(address(router)));
        vm.expectRevert(TreasuryV2.RouterNotWhitelisted.selector);
        treasury.buybackV3ExactInputSingle(address(other), address(router), 1, 0, POOL_FEE, false);
    }
    ...
    function testBuybackV3_DistributeRewardsTrue_TreasuryK613Zero() public {
        TreasuryV2 freshTreasury = new TreasuryV2(address(k613), address(xk613), address(staking), address(distributor));
    ...
    function testBuybackV3_BuybackFailed() public {
        ...
        k613.mint(address(router), ONE);
        treasury.setRouterWhitelist(address(router), true);
        vm.expectRevert(TreasuryV2.BuybackFailed.selector);
        treasury.buybackV3ExactInputSingle(address(other), address(router), 1, 0, POOL_FEE, false);
    }

    /// @notice test_Treasury_DepositRewards_ToRD_Claim: TreasuryV2 depositRewards stakes K613 and notifies RD; user with RD deposit can claim and receives expected xK613.
    ...
    function testConstructorZeroAddressReverts() public {
        vm.expectRevert(TreasuryV2.ZeroAddress.selector);
        new TreasuryV2(address(0), address(xk613), address(staking), address(distributor));

        vm.expectRevert(TreasuryV2.ZeroAddress.selector);
        new TreasuryV2(address(k613), address(0), address(staking), address(distributor));

        vm.expectRevert(TreasuryV2.ZeroAddress.selector);
        new TreasuryV2(address(k613), address(xk613), address(0), address(distributor));

        vm.expectRevert(TreasuryV2.ZeroAddress.selector);
        new TreasuryV2(address(k613), address(xk613), address(staking), address(0));
    }
