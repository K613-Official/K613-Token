...
contract FullLendingEconomyCycleV2 is Script, StdCheats {
    ...
    function _deployNearProd(address deployer) internal returns (ProtocolState memory p) {
        ...
        StakingV2 staking =
            new StakingV2(address(k613), address(xk), DEPLOY_LOCK_DURATION, DEPLOY_INSTANT_EXIT_PENALTY_BPS);
        RewardsDistributor rd = new RewardsDistributor(address(xk), address(xk), address(k613), DEPLOY_EPOCH_DURATION);
        TreasuryV2 treasury = new TreasuryV2(address(k613), address(xk), address(staking), address(rd));
        ...
        if (block.chainid == ARBITRUM_SEPOLIA) {
            console.log(
                "TreasuryV2.buybackV3ExactInputSingle needs SwapRouter02 exactInputSingle; whitelist manually via setRouterWhitelist."
            );
        }

        console.log("Deployed K613:", address(k613));
        console.log("Deployed xK613:", address(xk));
        console.log("Deployed StakingV2:", address(staking));
        console.log("Deployed RewardsDistributor:", address(rd));
        console.log("Deployed TreasuryV2:", address(treasury));
    ...
    function _loadProtocolFromEnv() internal view returns (ProtocolState memory) {
        return ProtocolState({
            ...
            staking: StakingV2(vm.envAddress("STAKING_ADDRESS")),
            rd: RewardsDistributor(vm.envAddress("REWARDS_DISTRIBUTOR_ADDRESS")),
            treasury: TreasuryV2(vm.envAddress("TREASURY_ADDRESS")),
    ...
    function _stakingExitableXk613(StakingV2 staking, address user) internal view returns (uint256) {
        (uint256 staked, StakingV2.ExitRequest[] memory queue) = staking.deposits(user);
        uint256 inQueue;
    ...
    function _phase1Bootstrap(
        ...
        // On a fork the admin rarely holds enough of the market's assets to seed two users, and
        // running out mid-run aborts the whole lifecycle before anything interesting is exercised.
        // `deal` only affects the local fork state, so this is a no-op for a real broadcast — but
        // guard on it anyway so a --broadcast run can never silently depend on fabricated balances.
        if (!_isBroadcasting()) {
            uint256 needSupply = (userASupply + userBCollateral) * 10;
            uint256 needBorrow = userBBorrow * 10;
            if (IERC20(protocol.supplyToken).balanceOf(admin) < needSupply) {
                deal(protocol.supplyToken, admin, needSupply);
                console.log("[fork] dealt supply asset to admin:", needSupply / (10 ** supplyDecimals));
            }
            if (IERC20(protocol.borrowToken).balanceOf(admin) < needBorrow) {
                deal(protocol.borrowToken, admin, needBorrow);
                console.log("[fork] dealt borrow asset to admin:", needBorrow / (10 ** borrowDecimals));
            }
        }
        ...
        vm.startBroadcast(userAPk);
        _logMoveAuto(
            "User A -> StakingV2 (stake pull K613)",
        ...
        vm.startBroadcast(userBPk);
        _logMoveAuto(
            "User B -> StakingV2 (stake pull K613)",
            address(protocol.k613),
            userB,
            address(protocol.staking),
            userBStake
    ...
    function _phase2LiquidityAndTreasury(
        ...
        if (userBxK613 > 0 && exitableStake > 0) {
            ...
            if (exitAmount > 0) {
                ...
                vm.startBroadcast(userBPk);
                _logMoveAuto(
                    "User B -> StakingV2 (xK613 initiateExit escrow)",
                    address(protocol.xk613),
                    userB,
                    address(protocol.staking),
                    exitAmount
        ...
        if (protocol.k613.balanceOf(admin) > ADMIN_BUYBACK_STAKE) {
            _logMoveAuto(
                "Admin -> StakingV2 (buyback stand-in stake)",
                address(protocol.k613),
                admin,
                address(protocol.staking),
                ADMIN_BUYBACK_STAKE
        ...
        console.log("--- TreasuryV2: depositRewards ---");
        vm.stopBroadcast();
        vm.startBroadcast(adminPk);
        _logMoveAuto(
            "Admin -> TreasuryV2 (K613 via transferFrom in depositRewards)",
            address(protocol.k613),
            admin,
        ...
        _logMoveAuto(
            "TreasuryV2 -> RewardsDistributor (xK613 after internal stake)",
            address(protocol.xk613),
            address(protocol.treasury),
            address(protocol.rd),
            TREASURY_REWARD
        );
        console.log("TreasuryV2 reward K613:", TREASURY_REWARD / 1 ether);
    ...
    function _maybeTreasuryBuyback(ProtocolState memory protocol, uint256 adminPk) internal {
        ...
        if (amount == 0) {
            console.log("FULL_LENDING_BUYBACK: skip zero TOKEN balance on TreasuryV2");
            return;
        }
        uint256 minK613Out = vm.envUint("MIN_K613_OUT");
        console.log("--- Optional: TreasuryV2 buyback V3 (same env as TreasuryBuybackV3) ---");
        vm.stopBroadcast();
        vm.startBroadcast(adminPk);
    ...
    function _phase3StakingClaimsAndExitStart(
        ...
        if (protocol.k613.balanceOf(admin) > ADMIN_BUYBACK_STAKE) {
            _logMoveAuto(
                "Admin -> StakingV2 (buyback stand-in repeat)",
                address(protocol.k613),
                admin,
                address(protocol.staking),
                ADMIN_BUYBACK_STAKE
        ...
        internal
    {
        ...
        for (uint256 iter = 0; iter < 24; ++iter) {
            ...
            if (xBal == 0) {
                console.log("StakingV2 drain stalled: staked>0 but wallet xK613=0 user:", user);
                break;
            }
            ...
            vm.startBroadcast(userPk);
            _logMoveAuto(
                "user -> StakingV2 (xK613 initiateExit for instant chunk)",
                address(protocol.xk613),
                user,
                address(protocol.staking),
                chunk
            ...
            console.log("StakingV2 instant exit chunk user:", user);
            vm.stopBroadcast();
            vm.startBroadcast(adminPk);
        }

        // V2: reward xK613 has no separate redemption path. Whatever the user holds beyond what
        // is already queued exits through the ordinary queue — the same door, terms and penalty as
        // self-staked xK613. That equivalence is the entire point of V2, so exercise it here rather
        // than the V1 `redeemRewards` side door, which no longer exists.
        _warpUntilEpochReady(protocol.rd);
        try protocol.rd.advanceEpoch() {
            console.log("advanceEpoch before reward exit: OK (pending K613 staked)");
        } catch {
            console.log("advanceEpoch before reward exit: skipped");
        }
        ...
        if (xWalletFinal > 0) {
            vm.stopBroadcast();
            vm.startBroadcast(userPk);
            console.log("--- reward xK613 exits via the ordinary queue (V2) ---");
            console.log("  user:", user);
            console.log("  xK613 wallet (reward + leftover stake):", xWalletFinal / 1 ether);
            console.log("  already queued:", protocol.staking.exitPendingSum(user) / 1 ether);

            uint256 kPreExit = protocol.k613.balanceOf(user);
            x.approve(address(protocol.staking), xWalletFinal);
            try protocol.staking.initiateExit(xWalletFinal) {
                uint256 idx = protocol.staking.exitQueueLength(user) - 1;
                // Instant path: proves the reward portion pays the same penalty as any other exit.
                try protocol.staking.instantExit(idx) {
                    uint256 kPostExit = protocol.k613.balanceOf(user);
                    _logBalanceDelta(
                        "user after reward instantExit (K613)", address(protocol.k613), user, kPreExit, kPostExit
                    );
                    console.log("  received K613 (50% penalty applied):", (kPostExit - kPreExit) / 1 ether);
                } catch {
                    console.log("  instantExit reverted -- request left queued");
                }
            } catch {
                console.log("  initiateExit reverted -- nothing exitable");
            }
            vm.stopBroadcast();
            vm.startBroadcast(adminPk);
        }
    ...
    function _drainExitQueuePreferInstant(StakingV2 staking, address user, uint256 userPk, uint256 adminPk) internal {
        uint256 guard = 0;
    ...
    function _phase5AaveEvacuate(
        ...
        // Aave can legitimately refuse this: code 26 when there is no aToken left to pull, code 35
        // when leftover debt would push the health factor under the liquidation threshold. Neither
        // says anything about the K613 side, so don't let it abort the run before the final
        // snapshot — which is the part that actually reports whether the cycle balanced.
        // Check before calling rather than try/catch: inside a broadcast the call is recorded for
        // replay even when it reverts, and forge then fails the run re-simulating it standalone.
        if (IERC20(protocol.aavePool.getReserveData(protocol.supplyToken).aTokenAddress).balanceOf(userB) > 0) {
            ...
            console.log("User B withdraw supply: no aToken balance -- skipped");
        }
        vm.stopBroadcast();
        ...
        if (IERC20(protocol.aavePool.getReserveData(protocol.supplyToken).aTokenAddress).balanceOf(userA) > 0) {
            ...
            console.log("User A withdraw supply: no aToken balance -- skipped");
        }
        vm.stopBroadcast();
        vm.startBroadcast(adminPk);
    ...
    function _printStepZeroSnapshot(ProtocolState memory protocol, address admin) internal view {
        ...
        console.log("StakingV2:", address(protocol.staking));
        console.log("RewardsDistributor:", address(protocol.rd));
        console.log("TreasuryV2:", address(protocol.treasury));
        ...
        console.log("TreasuryV2 K613:", protocol.k613.balanceOf(tr) / 1 ether);
        console.log("TreasuryV2 xK613:", IERC20(address(protocol.xk613)).balanceOf(tr) / 1 ether);
        console.log("TreasuryV2 supply asset:", supT.balanceOf(tr) / (10 ** sDec));
        console.log("TreasuryV2 borrow asset:", borT.balanceOf(tr) / (10 ** bDec));
        console.log("StakingV2 K613:", protocol.k613.balanceOf(st) / 1 ether);
        console.log("StakingV2 xK613:", IERC20(address(protocol.xk613)).balanceOf(st) / 1 ether);
        console.log("StakingV2 totalBacking:", protocol.staking.totalBacking() / 1 ether);
    ...
    function _printDustSnapshot(ProtocolState memory protocol, address admin) internal view {
        ...
        console.log("TreasuryV2 K613:", protocol.k613.balanceOf(address(protocol.treasury)) / 1 ether);
        console.log(
            "TreasuryV2 xK613:", IERC20(address(protocol.xk613)).balanceOf(address(protocol.treasury)) / 1 ether
        );
        console.log("StakingV2 K613:", protocol.k613.balanceOf(address(protocol.staking)) / 1 ether);
        console.log("StakingV2 xK613:", IERC20(address(protocol.xk613)).balanceOf(address(protocol.staking)) / 1 ether);
        console.log("StakingV2 totalBacking:", protocol.staking.totalBacking() / 1 ether);
    ...
    function _printSnapshot(string memory label, ProtocolState memory protocol, address admin) internal view {
        ...
        console.log("StakingV2 backing:", protocol.staking.totalBacking() / 1 ether);
        console.log("RD totalDeposits:", protocol.rd.totalDeposits() / 1 ether);
        console.log("K613 totalSupply:", IERC20(address(protocol.k613)).totalSupply() / 1 ether);
        console.log("xK613 totalSupply:", IERC20(address(protocol.xk613)).totalSupply() / 1 ether);
    }

    /// @dev True when the run will actually send transactions. Used to keep fork-only fixtures
    ///      (balance dealing) from ever influencing a real broadcast.
    function _isBroadcasting() internal view returns (bool) {
        return vm.envOr("FULL_LENDING_BROADCAST", false);
    }
}
